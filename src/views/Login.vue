	...
	<div class="login">
		...
		<div class="right-section">
			...
			<div v-if="isChoosingAvatar" class="avatar-selection">
				...
				<div class="avatars">
					<img v-for="avatar in avatars" :key="avatar" :src="'src/assets/avatar/' + avatar + '.svg'" :alt="avatar" @click="selectAvatar(avatar)" :class="{ selected: selectedAvatar === avatar }" />
				</div>
				<div class="login-btn" @click="chooseAvatar">Valider</div>
			</div>
			<div class="footer">
				<img class="logo-ministere" src="../assets/logo_ministere.png" alt="Ministère de l'Éducation" />
			</div>
		</div>
	</div>
</template>
					...
					if (!querySnapshot.empty) {
						...
						if (!userData.register) {
							...
							if (userData.password === this.password) {
								...
								localStorage.setItem('user', JSON.stringify({ ...userData, password: hashedPassword }));
							...
							const validPassword = await bcrypt.compare(this.password, userData.password);
